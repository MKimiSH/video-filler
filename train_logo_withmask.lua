...
opt = {
   batchSize = 2,         -- number of samples to produce, 2 IS FOR TESTING!!
   ...
   --MmaskName = '/media/mkimish/Nothing/TV_logo/mask.png' -- name of mask, not only square --M
   -- Extra Options:
   withMask = 1,           --M only in this script it is not --nil-- If not nil, then return data AND mask
...
-- create data loader
local DataLoader = paths.dofile('data/data_logo.lua')
...
netG:add(SpatialBatchNormalization(ngf/2)):add(nn.ReLU(true)) --M
--M state size: (ngf/2) x 64 x 64
...
Let's see what will happen.
  One thing: should the nn structure be changed?????
]]--
...
local fDx = function(x)
   ...
   local real_ctx, real_full = data:getBatch() --M real_ctx is masked out, while real_full is not.

--[[M   local real_center = real_ctx[{{},{},{1 + opt.fineSize/4, opt.fineSize/2 + opt.fineSize/4},{1 + opt.fineSize/4, opt.fineSize/2 + opt.fineSize/4}}]:clone() -- copy by value, default fineSize=128
   ...
   real_ctx[{{},{3},{1 + opt.fineSize/4 + opt.overlapPred, opt.fineSize/2 + opt.fineSize/4 - opt.overlapPred},{1 + opt.fineSize/4 + opt.overlapPred, opt.fineSize/2 + opt.fineSize/4 - opt.overlapPred}}] = 2*123.0/255.0 - 1.0 -- set RGB channels with mean values
]]--
...
local fGx = function(x)
   ...
   if opt.wtl2~=0 then
      ...
      local df_dg_l2 = criterionMSE:backward(input_inpainted, input_real)

      if opt.overlapPred==0 then --M overlapPred should should should be 0 in first test!!!
...
for epoch = 1, opt.niter do
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      if counter % opt.display_iter == 0 and opt.display then
          local real_ctx, real_full = data:getBatch()
          ...
          --M real_ctx[{{},{},{1 + opt.fineSize/4 + opt.overlapPred, opt.fineSize/2 + opt.fineSize/4 - opt.overlapPred},{1 + opt.fineSize/4 + opt.overlapPred, opt.fineSize/2 + opt.fineSize/4 - opt.overlapPred}}]:copy(fake[{{},{},{1 + opt.overlapPred, opt.fineSize/2 - opt.overlapPred},{1 + opt.overlapPred, opt.fineSize/2 - opt.overlapPred}}])
          disp.image(fake, {win=opt.display_id, title=opt.name})
          disp.image(real_full, {win=opt.display_id * 3, title=opt.name})
          --M disp.image(real_ctx, {win=opt.display_id * 6, title=opt.name})
      end
