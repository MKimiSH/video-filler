...
function dataset:__init(...)
   ...
   local s_data = self.imagePath:data() --M: s_data is the pointer to self.imagePath
   local count = 0
...
--M 1-gt, 2-masked (hopefully)
function dataset:getTwoImages()
  -- the two classes should have same number of samples
  assert(self.classListSample[1]:nElement() == self.classListSample[2]:nElement())
  local class = 1
  local index = math.ceil(torch.uniform() * self.classListSample[class]:nElement())
  local pathgt = ffi.string(torch.data(self.imagePath[self.classListSample[class][index]]))
  local pathmasked = ffi.string(torch.data(self.imagePath[self.classListSample[class+1][index]]))
  --print(pathgt)
  --print(pathmasked)
  local retmasked, retgt = self:sampleHookTrain(pathmasked, pathgt)
  return retmasked, retgt, index
end

--M Indentation with 2 spaces is acceptable anyway
--M and easy to find that it is by me.
-- A variance of dataset:sample which samples from 2 different classes
-- simultaneously and with the same index (so one is masked and the other
-- is the corresponding full image).
function dataset:sample2(quantity)
  assert(quantity)
  -- print('quantity=', quantity)
  local maskedTable = {}
  local gtTable = {}
  local indexTable = {}
  --M 1-gt, 2-masked (hope so)
  for i = 1,quantity do
    local outmasked, outgt, idx = self:getTwoImages()
    table.insert(maskedTable, outmasked)
    -- print('size, ', outmasked:size())
    table.insert(gtTable, outgt)
    table.insert(indexTable, idx)
  end
  -- print('length of mtable, ', #maskedTable)
  local mdata, gdata, idxdata
  mdata, idxdata = tableToOutput(self, maskedTable, indexTable)
  gdata, idxdata = tableToOutput(self, gtTable, indexTable)
  return mdata, gdata
end
