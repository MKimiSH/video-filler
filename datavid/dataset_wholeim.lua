--[[
    MKimiSH: This version samples a piece of video with multiple patches
    Currently the video length should be 1!! (0907)
...
function dataset:__init(...)
   ...
   print(args)
   for k,v in pairs(args) do self[k] = v end --M; print(k, v) end --M
   ...
   -- for each class,
   --M classPaths is a 2-d array of strings.
   for k,path in ipairs(self.paths) do
      --M print(path) --M
      local dirs = dir.getdirectories(path); --M pl.dir, return a list of all subdirectories of the directory.
      for k,dirpath in ipairs(dirs) do
         local class = paths.basename(dirpath) --M basename: remove prefix of a filename
  	 --M print('class = ', class)
   ...
   --M os.tmpname(): generate a temp file and return its name
   ...
   for i, class in ipairs(self.classes) do
      ...
      for j,path in ipairs(classPaths[i]) do
         local command = find .. ' "' .. path .. '" ' .. findOptions
            .. ' | sort >>"' .. classFindFiles[i] .. '" \n'
	 --M print(command)
         tmphandle:write(command)
      end
   end
   io.close(tmphandle)
   --os.execute('emacs ' .. tmpfile)
   ...
   for line in io.lines(combinedFindList) do
      ffi.copy(s_data, line)  --M ffi.copy(dst, src[, len])
   ...
   for i=1,#self.classes do
      ...
      if length == 0 then
         ...
         --M fill self.classList with sample indices of corresponding class (2-d)
         --M fill self.imageClass with class index of the samples (1-d)
   ...
   --os.execute('emacs "' .. combinedFindList .. '"')
   --==========================================================================
...
function dataset:getByClass(class)
   --M this torch.uniform() is the origin of the random sampling.!!!!!!!!!
   local index = math.ceil(torch.uniform() * self.classListSample[class]:nElement())
   --M ffi.string - transfer a C pointer to a Lua string
   --M self.classListSample[class][index] draws the absolute index of the sample.
   local imgpath = ffi.string(torch.data(self.imagePath[self.classListSample[class][index]]))
   --M print(imgpath)
   return self:sampleHookTrain(imgpath)
end

--[[M The ideal functionality of getVid() should be:
...
function dataset:getVid(class, quantity)
  ...
  -- I am going to hardcode my dataset into this...
  ...
  vid = 1
  local frame = torch.random(nums[vid] - quantity)
  local frNames = {}
  for i=1,quantity do
    local frName = base .. string.format('%07d', frame+i-1) .. ext[vid]
    frNames[#frNames+1] = frName
  end
  return self:sampleHookTrain(frNames, 0)
end

--M getVid returns (quantity x nc x w x h) which is the output
--M getVideoClip returns [(length x nc) x w x h] to insert into a Table
function dataset:getVideoClip(class, length)
  local ext = {'_hd1.jpg', '_hd2.jpg', '_hd3.jpg', '_hd4.jpg'}
  -- local ext = {'.jpg', '.jpg', '.jpg', '.jpg'}
  local nums = {41573, 41144, 41432, 34376}
  local base = opt.data .. '/gt/' -- /masked/ can be of no use...
  if torch.uniform() > 0.6 then
    base = opt.data .. '/gt1/'
  end
  local vid = torch.random(4)
  -- vid = 1
  local frame = torch.random(nums[vid] - length)
  local frNames = {}
  for i=1,length do
    local frName = base .. string.format('%07d', frame+i-1) .. ext[vid]
    -- local frName = base .. 'train_' .. string.format('%07d', frame+i-1) .. ext[vid]
    frNames[#frNames+1] = frName
  end
  return self:sampleHookTrain(frNames, 1)
end
...
local function tableToOutput(self, dataTable, scalarTable)
   ...
   data = torch.Tensor(quantity, dataTable[1]:size(1), dataTable[1]:size(2), dataTable[1]:size(3))
...
--M define a new sample() whose task is quite easy: generate ONE ramdom number
--M and use it as the first frame.
function dataset:sample(quantity, length)
  assert(quantity, 'predLen is nil')
  assert(length==1, 'this version requires length==1')
  --M loop over quantity
   local dataTable = {}
   local scalarTable = {}
   local maskTable = {}
   local maskedTable = {}
   -- print('classes num=', #self.classes) -- ADDED!!
   local cnt, total = 0,0
   while true do
       total = total+1
       local class = torch.random(1, #self.classes)
       local out, outmask, masked = self:getVideoClip(class, length) -- out is (ncxlength) x w x h
       if out then
         table.insert(dataTable, out)
         table.insert(maskTable, outmask)
         table.insert(maskedTable, masked)
         table.insert(scalarTable, class)
         cnt = cnt+1
         if cnt >= quantity then
           break
         end
       end
   end
   print('went through ' .. total .. ' images.')
  ...
  --[[M for i=1,quantity do
    local class = torch.random(1, #self.classes)
    local data = self:getVid(class, quantity, length)
    local scalarLabels = torch.LongTensor(quantity):fill(class)
  end
...
function dataset:sample_obsolete(quantity)
   assert(quantity)
   local dataTable = {}
   local scalarTable = {}
   -- print('classes num=', #self.classes) -- ADDED!!
