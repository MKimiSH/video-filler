...
local function tableToOutput(self, dataTable, scalarTable)
   ...
   data = torch.Tensor(quantity, dataTable[1]:size(1), dataTable[1]:size(2), dataTable[1]:size(3), dataTable[1]:size(4))
...
function dataset:sample(quantity, length)
  ...
  --M loop over quantity
   ...
   while true do
       ...
       local out, outmask, masked = self:getVideoClip(class, length) -- out is nc x length x w x h
       if out then
